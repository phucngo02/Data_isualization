...
########### Mô hình 2 đầu
########### Bao gồm left encoder, right encoder
...
class Shared_Encoder(nn.Module):
    def __init__(self, input_dim, enc_hid_dim, dec_hid_dim, enc_layers, dec_layers, dropout_p):
        ...
        self.gru_right = nn.GRU(input_size=self.enc_hid_dim,
                                ...
                                num_layers=self.enc_layers,
                                bidirectional=True)
        self.output_linear_left = nn.Linear(self.enc_hid_dim * 2,self.dec_hid_dim)
        self.output_linear_right = nn.Linear(self.enc_hid_dim * 2, self.dec_hid_dim)
    ...
    def forward(self, input_before, input_after):
        ...
        hidden_before = torch.tanh(
            self.output_linear_left(torch.cat((hidden_before[-2, :, :], hidden_before[-1, :, :]), dim=1)))
        ...
        hidden_after = torch.tanh(self.output_linear_right(torch.cat((hidden_after[-2, :, :], hidden_after[-1, :, :]), dim=1)))
...
class Cross_Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        # Chỉ lấy lớp ẩn cuối cùng từ decoder
...
class Decoder(nn.Module):
    def __init__(self, output_dim, enc_hid_dim, dec_hid_dim, dec_layers,dropout_p, attention):
        ...
        self.gru = nn.GRU(input_size=self.enc_hid_dim * 2 + self.dec_hid_dim,hidden_size=self.dec_hid_dim,  num_layers=self.dec_layers)

        self.out = nn.Linear(self.enc_hid_dim * 2 + self.dec_hid_dim + self.dec_hid_dim,self.output_dim)
        self.dropout = nn.Dropout(self.dropout_p)

    def forward(self, input, hidden, encoder_outputs_left, encoder_outputs_right):
        ...
        encoder_outputs = torch.cat((encoder_outputs_left, encoder_outputs_right), dim=0)
...
class DualSSIM(nn.Module):
    ...
    def forward(self, src_left, src_right, trg, teacher_forcing_ratio=0.5):
        ...
        # Lưu trạng thái attention
        decoder_attn = torch.zeros(max_len, batch_size, 20).to(self.device)

        # Shared Encoder
        encoder_outputs_left, encoder_outputs_right, hidden = self.shared_encoder(src_left, src_right)
